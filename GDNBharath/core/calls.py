...
from GDNBharath.helpers.buttons import Buttons
from GDNBharath.helpers.strings import TEXTS
from GDNBharath.utils.exceptions import (
    ChangeVCException,
    JoinGCException,
    JoinVCException,
    UserException,
)
from GDNBharath.utils.queue import Queue
from GDNBharath.utils.thumbnail import thumb
from GDNBharath.utils.youtube import ytube

from .clients import GDNbot
...
class GDNBharath(PyTgCalls):
    ...
    async def autoend(self, chat_id: int, users: list):
        ...
        if autoend:
            if len(users) == 1:
                get = await GDNbot.app.get_users(users[0])
                if get.id == GDNbot.user.id:
    ...
    async def change_vc(self, chat_id: int):
        ...
        try:
            user = (await GDNbot.app.get_users(user_id)).mention(style="md")
        except:
            user = get[0]["user"]
        if queue:
            ...
            try:
                ...
                if photo:
                    ...
                    sent = await GDNbot.app.send_message(
                        int(chat_id),
                        TEXTS.PLAYING.format(
                            GDNbot.app.mention,
                            title,
    ...
    async def join_gc(self, chat_id: int):
        try:
            try:
                get = await GDNbot.app.get_chat_member(chat_id, hellbot.user.id)
            ...
            chat = await GDNbot.app.get_chat(chat_id)
            if chat.username:
                try:
                    await GDNbot.user.join_chat(chat.username)
                ...
                try:
                    try:
                        ...
                        if link is None:
                            link = await GDNbot.app.export_chat_invite_link(chat_id)
                    except ChatAdminRequired:
                        raise UserException(
                            f"[UserException]: Bot is not admin in chat {chat_id}"
                        )
                        ...
                        raise UserException(f"[UserException]: {e}")
                    hell = await GDNbot.app.send_message(
                        chat_id, "Inviting assistant to chat..."
                    )
                    if link.startswith("https://t.me/+"):
                        link = link.replace("https://t.me/+", "https://t.me/joinchat/")
                    await GDNbot.user.join_chat(link)
                    await GDN.edit_text("Assistant joined the chat! Enjoy your music!")
                except UserAlreadyParticipant:
                    pass
                except Exception as e:
                    raise UserException(f"[UserException]: {e}")
