...
from GDNBharath.core.calls import GDNBHARATH 
from GDNBharath.core.clients import GDNbot
from GDNBharath.core.database import db
...
class Player:
    ...
    async def play(self, message: Message, context: dict, edit: bool = True):
        ...
        if force:
            await GDNBharath.leave_vc(chat_id, True)
    ...
    async def replay(self, chat_id: int, message: Message):
        ...
        if que["file"] == que["video_id"]:
            ...
            file_path = que["file"]
        try:
            await GDNBharath.replay_vc(chat_id, file_path, video)
        except Exception as e:
            await message.delete()
            await message.reply_text(str(e))
            Queue.clear_queue(chat_id)
            ...
            try:
                ...
                if count == 0 and previously == 0:
                    ...
                    try:
                        ...
                        await GDNBHARATH.join_vc(message.chat.id, file_path, video)
                    except Exception as e:
                        await message.edit_text(str(e))
